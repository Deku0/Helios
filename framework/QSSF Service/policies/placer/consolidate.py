    ...
    def place(self, job):
        ...
        if select_flag:
            for (node, req_gpu) in alloc_nodes:
                # test log
                # print("node %d has total %d gpus, now there are %d jobs and %d free gpus, want to cost %d gpus"
                #         %(node.get_node_name(), node.get_num_gpus(), node.get_node_job_num(), node.get_free_gpus(),req_gpu))
